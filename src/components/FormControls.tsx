...
export function Action({ label, onPress, disabled = false, secondary = false }: { label: string; onPress: () => void; disabled?: boolean; secondary?: boolean }) {
  return <Pressable accessibilityRole="button" accessibilityState={{ disabled }} disabled={disabled} onPress={onPress} className={`mb-3 min-h-12 justify-center rounded-xl px-4 py-3 ${disabled ? 'bg-zinc-300' : secondary ? 'bg-zinc-200' : 'bg-red-700'}`}><Text className={`text-center font-semibold ${secondary ? 'text-zinc-900' : 'text-white'}`}>{label}</Text></Pressable>;
}
export function Choice({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) {
  return <Pressable accessibilityRole="radio" accessibilityState={{ checked: selected }} onPress={onPress} className={`mb-2 mr-2 min-h-12 justify-center rounded-xl border px-4 py-3 ${selected ? 'border-red-700 bg-red-50' : 'border-zinc-300 bg-white'}`}><Text className={selected ? 'font-semibold text-red-800' : 'text-zinc-700'}>{label}</Text></Pressable>;
}
