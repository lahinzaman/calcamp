...
const transition={duration:()=>transition,delay:()=>transition,reduceMotion:()=>undefined};
mock.module('react-native-reanimated',{defaultExport:{View:'AnimatedView',createAnimatedComponent:(c:unknown)=>c},namedExports:{ReduceMotion:{System:'system'},FadeInDown:transition,FadeOutDown:transition,LinearTransition:transition,useReducedMotion:()=>false,useSharedValue:(value:unknown)=>React.useRef({value}).current,useAnimatedStyle:(fn:()=>unknown)=>fn(),withTiming:(v:unknown)=>v}});
