...
export default function GymStatus() {
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} className="flex-1 bg-background"><ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 50, maxWidth: 760, width: '100%', alignSelf: 'center' }}>
    ...
    <Text className="mb-6 text-ink">BestTime estimates visits relative to a venue's weekly peak. Recent student reports take priority. Neither measures physical capacity or confirms opening hours.</Text>
    ...
    {GYMS.map(gym => {
      ...
      return <View key={gym.slug} className="mb-5 rounded-3xl border border-border bg-surface p-5">
        ...
        <Text className="my-3 text-3xl font-bold text-ink">{score === null ? 'No estimate' : `${Math.round(score)} / 100`}</Text>
        <Text className="mb-3 text-ink">{override ? `${report.vote_count} student report${report.vote_count === 1 ? '' : 's'} · ${score! < 25 ? 'Quiet' : score! < 75 ? 'Normal' : 'Packed'}` : history?.stale ? 'Saved estimate · waiting for a live update' : 'BestTime historical busyness'}</Text>
    ...
    {baseline.isPending && <Text className="mb-3 text-ink">Loading forecasts…</Text>}
    {baseline.isError && <Text className="mb-3 text-ink">Forecasts are unavailable; student reports can still be used.</Text>}
