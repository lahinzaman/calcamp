  ...
  return <SafeAreaView edges={['top', 'left', 'right']} className="flex-1 bg-zinc-50"><ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 50, maxWidth: 760, width: '100%', alignSelf: 'center' }}>
    ...
    {GYMS.map(gym => {
      ...
      return <View key={gym.slug} className="mb-5 rounded-3xl border border-zinc-200 bg-white p-5">
        ...
        <Text className="mb-3 text-zinc-600">{override ? `${report.vote_count} student report${report.vote_count === 1 ? '' : 's'} · ${score! < 25 ? 'Quiet' : score! < 75 ? 'Normal' : 'Packed'}` : history?.stale ? 'Saved estimate · waiting for a live update' : 'BestTime historical busyness'}</Text>
    ...
    {crowd.isError && <Text className="mb-3 text-red-700">Student reports are unavailable. Check your connection and refresh.</Text>}
