...
export default function DashboardScreen() {
  ...
  return <SafeAreaView edges={['top','left','right','bottom']} className="flex-1 bg-background"><ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 110, maxWidth: 760, width: '100%', alignSelf: 'center' }}>
    <Text className="text-sm font-bold tracking-widest">CALCAMP</Text><Text className="mt-3 text-4xl leading-[52px] font-bold">Your daily picture.</Text><Text className="mt-1">{date} · Nutrition & training, together</Text>
    <AccountAccess /><SyncIndicator /><MacroOverview />
    <View className="mb-5 rounded-3xl bg-surface p-5"><Text className="text-xl font-bold">4-day Upper / Lower</Text><Text className="mb-4 mt-1">{session ? `In progress · ${session.name}` : 'A clear plan. One session at a time.'}</Text>
    ...
    <Action label="🚶 Plan a walk" secondary onPress={() => router.push('/walk')} />
