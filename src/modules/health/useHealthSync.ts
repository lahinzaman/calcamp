...
export function createHealthStore(loadAdapter: () => Promise<HealthAdapter> = getHealthAdapter, now = () => new Date()) {
  ...
  return createStore<HealthState>()((set, get) => ({
    ...
    initialize: async () => {
      ...
      initializing = true; set({ status: 'initializing', error: null });
      try { adapter = await loadAdapter(); await adapter.initialize(); set({ initialized: true }); await get().refresh(); }
      catch (error) { set({ status: 'error', initialized: false, error: error instanceof Error ? error.message : 'Health sync is unavailable.' }); }
      finally { initializing = false; }
    },
    refresh: async () => {
      if (!adapter || !get().initialized || reading) return;
      reading = true; const date = localDay(now()).date;
      if (get().date !== date) set({ date, steps: null, activeEnergyKcal: null, refreshedAt: null });
      try {
        const summary = await adapter.readToday(now());
        ...
        set({ ...summary, status: 'ready', error: summary.steps === null ? 'Step read access is unavailable.' : null, refreshedAt: now().getTime() });
      } catch (error) { set({ status: 'error', steps: null, activeEnergyKcal: null, error: error instanceof Error ? error.message : 'Health read failed.' }); }
