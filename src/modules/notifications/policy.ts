...
export function parsePreferences(value: unknown): NotificationPreferences {
  ...
  for (const k of ['enabled','gymAlerts','workoutReminders','nutritionReminders','geofencing','uploadActivity'] as const) if (typeof p[k] !== 'boolean') throw new Error('Choose valid reminder settings.');
  ...
  if (![p.workoutTime, p.nutritionTime].every(t => /^([01]\d|2[0-3]):[0-5]\d$/.test(t))) throw new Error('Use a time such as 17:00.');
...
export function notificationRoute(data: unknown): NotificationRoute | null {
  ...
  return value?.kind === 'workout' ? '/(tabs)/explore' : value?.kind === 'nutrition' || value?.kind === 'rescue' ? '/(tabs)' : value?.kind === 'gym' ? '/(tabs)/campus' : null;
}
export interface Reminder { id: string; title: string; body: string; kind: 'workout' | 'nutrition'; hour: number; minute: number; weekday?: number }
