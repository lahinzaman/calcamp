export type TelemetryEvent = 'sync.queued' | 'sync.syncing' | 'sync.synced' | 'sync.retry' | 'sync.conflict' | 'health.permission' | 'health.merge' | 'health.conflict' | 'health.unavailable' | 'api.fallback' | 'notification.registration' | 'notification.received' | 'background.wake';
type SafeData = { outcome?: 'ok' | 'unavailable' | 'denied' | 'stale' | 'requested'; count?: number; source?: 'health' | 'nutrislice' | 'campus' | 'geofence' | 'push' };
