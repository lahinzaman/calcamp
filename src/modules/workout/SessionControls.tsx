...
export function SessionControls({ completedSets, volumeLbs, onFinish, onCancel }: {
  ...
  return <>
    <View className="mt-6 gap-3">
      <Pressable accessibilityRole="button" accessibilityLabel="Finish session" onPress={() => { setIntent('finish'); haptic('selection'); }}
        weight="firm" className="min-h-14 items-center justify-center rounded-2xl bg-accent px-4">
        <Text className="font-bold">Finish session</Text>
      </Pressable>
      <Pressable accessibilityRole="button" accessibilityLabel="Cancel session" onPress={() => { setIntent('cancel'); haptic('selection'); }}
        weight="subtle" tone="warning" className="min-h-12 items-center justify-center rounded-2xl border border-border px-4">
        <Text className="font-semibold">Cancel session</Text>
      </Pressable>
    </View>
    ...
    {intent && <Modal visible presentationStyle="pageSheet" animationType="slide" onRequestClose={close}>
      <SafeAreaView className="flex-1 bg-background"><ScrollView contentContainerStyle={{ padding: 24 }}>
        <Text className="mb-3 text-3xl font-bold">{intent === 'finish' ? 'Finish this session?' : 'Cancel this session?'}</Text>
        {intent === 'finish' ? <>
          ...
          <Action label="Finish and save" onPress={confirm} tone="success" />
        </> : <>
          <Text className="mb-5 leading-6">
            {completedSets === 0
          ...
          <Action label="Discard this session" onPress={confirm} tone="none" />
        </>}
        <Action secondary label="Keep training" onPress={close} />
